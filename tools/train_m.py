...
from datasets.ycb.dataset1 import PoseDataset as PoseDataset_ycb
...
if choose_net == 0:
    ...
    from lib.network_resUNet_edge import PoseNet, PoseRefineNet
elif choose_net == 2:
    from lib.network_resUNet_edge_pointp import PoseNet, PoseRefineNet
elif choose_net == 3:
    from lib.network_resUNet_edge_pointp_vlad_se_ori_refine import PoseNet, PoseRefineNet
elif choose_net == 4:
    from lib.network_UNet_edge import PoseNet, PoseRefineNet
from lib.loss_m import Loss
...
def main():
    ...
    if opt.dataset == 'ycb':
        ...
        opt.num_objects = 1
        opt.num_points = 1000
        opt.outf = '../trained_models/dbc'
        opt.log_dir = '../experiments/logs/dbc'
        opt.repeat_epoch = 15
    else:
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                loss, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, target, model_points, idx, points,
                                                              opt.w, opt.refine_start, edge, pred_edge, edge_weight,
                                                              weight_blance)
        ...
        for j, data in enumerate(testdataloader, 0):
            ...
            _, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, target, model_points, idx, points, opt.w,
                                                       opt.refine_start, edge, pred_edge, edge_weight, weight_blance)
