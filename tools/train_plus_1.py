...
from datasets.dbc.dataset import PoseDataset as PoseDataset_dbc
from lib.cosine_annealing_warmup import CosineAnnealingWarmupRestarts
from torch.optim.lr_scheduler import ReduceLROnPlateau

choose_net = 8
# 0-vanilla 1-trans 2-trans_pointp 3-trans_pointp_se 4-pointp 5-connecttrans 6-eff 7-eff_pointp 8-eff_pointp_vlad_se
# 9-kmeans 10-eff_pointp_vlad_kmeans_se
if choose_net == 0:
...
parser.add_argument('--w_rate', default=0.27, help='learning rate decay rate')
parser.add_argument('--decay_margin', default=0.021, help='margin to decay lr & w')
parser.add_argument('--refine_margin', default=0.00009, help='margin to start the training of iterative refinement')
...
parser.add_argument('--nepoch', type=int, default=900, help='max number of epochs to train')
...
def main():
    ...
    if opt.dataset == 'ycb':
        ...
        opt.repeat_epoch = 12
    elif opt.dataset == 'dbc':
        opt.num_objects = 1
        opt.num_points = 1000
        opt.outf = '/media/r/Samsung_USB/trained_models/dbc'
        opt.log_dir = '/media/r/Samsung_USB/experiments/logs/dbc'
        opt.repeat_epoch = 15
    else:
    ...
    if opt.resume_refinenet != '':
        ...
        optimizer = optim.AdamW(refiner.parameters(), lr=opt.lr)
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.72, patience=2, verbose=True, threshold=3e-5,
                                      threshold_mode='abs', cooldown=0, min_lr=5e-6, eps=1e-8)
    else:
        opt.refine_start = False
        opt.decay_start = False
        optimizer = optim.AdamW(estimator.parameters(), lr=opt.lr)
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.72, patience=2, verbose=True, threshold=3e-5,
                                      threshold_mode='abs', cooldown=0, min_lr=5e-6, eps=1e-8)
    ...
    if opt.dataset == 'ycb':
        ...
        dataset = PoseDataset_dbc('train', opt.num_points, True, opt.dataset_root, opt.noise_trans, opt.refine_start)
    ...
    if opt.dataset == 'ycb':
        ...
        test_dataset = PoseDataset_dbc('test', opt.num_points, False, opt.dataset_root, 0.0, opt.refine_start)
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        if best_test < opt.refine_margin and not opt.refine_start:
            ...
            if opt.dataset == 'ycb':
                ...
                dataset = PoseDataset_dbc('train', opt.num_points, True, opt.dataset_root, opt.noise_trans,
                                          opt.refine_start)
            dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=opt.workers)
            if opt.dataset == 'ycb':
                test_dataset = PoseDataset_ycb('test', opt.num_points, False, opt.dataset_root, 0.0, opt.refine_start)
