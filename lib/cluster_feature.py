...
class KMeans:
    ...
    def __init__(self, n_clusters, max_iter=6, tol=0.0001, verbose=0, mode="euclidean", minibatch=None):
...
class Cluster_Feature(nn.Module):
    def __init__(self, device, num_centers, dim, max_iterations):
        super(Cluster_Feature, self).__init__()

        self.device = device
        self.num_centers = num_centers

        self.k_means = KMeans(n_clusters=num_centers, max_iter=max_iterations)

        self.pool = nn.AdaptiveAvgPool1d(output_size=1)
        self.conv = torch.nn.Conv1d(dim, 256, 1)

    def forward(self, x):  # [1,D,N]
        x_code = x.clone()
        x_code = x_code.squeeze(0)
        x_code = x_code.transpose(1, 0).contiguous()  # [N,D]
        N, D = x_code.shape
        x_code = x_code.cpu()

        code = self.k_means.fit_predict(x_code)
        x_code = x_code.cuda()
        x_cluster = [[] for i in range(self.num_centers)]
        for i in range(N):
            x_cluster[code[i].item()].append(x_code[i])
        for index, value in enumerate(x_cluster):
            if len(value) != 0:
                value = torch.stack(tuple(value))  # [num,D]
                value = value.transpose(1, 0).contiguous().unsqueeze(0)  # [1,D,num]
                value = self.pool(value)  # [1,D,1]
                value = F.relu(self.conv(value))  # [1,256,1]
                x_cluster[index] = value
        cluster_feature = torch.empty(1, 256, 0).to(self.device)
        for i in range(N):
            cluster_feature = torch.cat([cluster_feature, x_cluster[code[i].item()]], -1)
        x = torch.cat([x, cluster_feature], 1)  # [1,D+256,N]
        return x
