...
class Upsample_Module(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            ...
            SKConvBlock(in_channels=in_channels, out_channels=out_channels, num_branches=3),
            nn.PReLU()
        )
...
class Encoder_Decoder_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x3, x4 = self.encoder(x)  # x3[B,256,H/16,W/16]    x4[B,512,H/32,W/32]
        ...
        x_psp = torch.cat([x3, x4], 1)  # 768,H/8,W/8
