...
from lib.Decoder_Net_Eff import Encoder_Decoder_Net
from lib.pointnet_plus_sknet_gap import SK_PointNet
from lib.NetVlad import NetVLADLoupe
from lib.SK_SE_Net import SEBlock
from lib.cluster_feature import Cluster_Feature
...
class PoseNet(nn.Module):
    def __init__(self, num_points, num_obj):
        ...
        self.num_points = num_points
        self.Encoder_Decoder = Encoder_Decoder_Net()
        ...
        self.cluster = Cluster_Feature(device=torch.device('cuda'), num_centers=6, dim=640, max_iterations=6)

        self.senet1 = SEBlock(channels=640)
        self.senet2 = SEBlock(channels=1920)
    ...
    def forward(self, img, x, choose, obj):
        out_img = self.Encoder_Decoder(img)
        ...
        ap_x = torch.cat([emb, feat_x], 1)  # 256+384=640
        ap_x = self.senet1(ap_x)

        cluster_feature_x = self.cluster(ap_x)

        ap_x = self.vlad(ap_x)
        ap_x = ap_x.view(-1, 1024, 1).repeat(1, 1, self.num_points)  # 构建全局特征

        ap_x = torch.cat([cluster_feature_x, ap_x], 1)  # 256+384+256+1024=1920

        ap_x = self.senet2(ap_x)
