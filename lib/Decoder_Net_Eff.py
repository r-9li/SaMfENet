...
from lib.Efficient_Net_V2 import EffNetV2
...
class Encoder_Decoder_Net(nn.Module):
    def __init__(self):
        ...
        self.encoder = EffNetV2()

        self.x_up = Upsample_Module(512, 512)

        self.psp = PSPModule(768, 1536, (1, 2, 3, 6))
    ...
    def forward(self, x):
        ...
        x1, x = self.encoder(x)  # x[B,256,H/8,W/8] x[B,512,H/16,W/16]

        x = self.x_up(x)  # 512,H/8,W/8

        x = F.upsample(input=x, size=(t_H, t_W), mode='bilinear')  # 尺寸统一

        x_psp = torch.cat([x1, x], 1)  # 768,H/8,W/8
