...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        edge_img = np.transpose(img_masked, (1, 2, 0))
        bool_label_masked = np.reshape(mask_label, (mask_label.shape[0], mask_label.shape[1], 1))
        bool_label_masked = np.repeat(bool_label_masked, 3, axis=-1)
        bool_label_masked = bool_label_masked[rmin:rmax, cmin:cmax, :]
